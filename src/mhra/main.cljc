(ns mhra.main
  "Kotodama WASM entrypoint for the Mhra clean-room actor (L4) — Clojure port.
  ...
  facts (`mhra.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"mhra.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mhra-compat" :tier tier :entities entities} 200])
